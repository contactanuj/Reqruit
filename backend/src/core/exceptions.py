...
class TierLimitExceededError(RateLimitError):
    """Raised when a user's weekly usage exceeds their tier limit (error 4250)."""
    ...
    def __init__(
        ...
        upgrade = "Upgrade to Pro for $15/week with 5M tokens"
        super().__init__(
            detail="Weekly usage tier limit exceeded",
            error_code="TIER_LIMIT_EXCEEDED",
        )
        self.upgrade_path = upgrade


class DailyCapReachedError(RateLimitError):
    """Raised when a user's daily cost exceeds the daily cap (error 4251)."""
