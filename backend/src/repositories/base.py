...
class BaseRepository(Generic[T]):
    ...
    async def find_by_ids(self, ids: list[PydanticObjectId]) -> list[T]:
        """Fetch multiple documents by their ObjectIds in a single query."""
        if not ids:
            return []
        return await self._model.find({"_id": {"$in": ids}}).to_list()
    ...
    async def delete_many(self, filters: dict[str, Any]) -> int:
        ...
        Uses Beanie's bulk delete (single round-trip) instead of loading
        each document individually. Returns the number of deleted documents.

        Args:
            filters: MongoDB query dict identifying documents to delete.
        ...
        """
        try:
            result = await self._model.find(filters).delete()
            return result.deleted_count if result else 0
        except Exception as e:
            raise DatabaseError(
                detail=f"Failed to delete {self._model.__name__} documents: {e}"
            ) from e
