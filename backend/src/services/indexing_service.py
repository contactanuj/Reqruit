...
class IndexingService:
    ...
    async def delete_job_embeddings(self, job_id: str, user_id: str) -> int:
        """Delete all JobEmbedding entries for a job from Weaviate.

        Non-blocking: logs a warning and returns 0 on failure.

        Args:
            job_id: The MongoDB ObjectId of the Job.
            user_id: The owner's user ID (Weaviate tenant).
        ...
        try:
            ...
            logger.warning(
                "delete_job_embeddings_failed",
                job_id=job_id,
                user_id=user_id,
            )
            return 0

    async def delete_cover_letter_embeddings_for_docs(
        self, doc_ids: list[str], user_id: str
    ) -> int:
        """Delete CoverLetterEmbedding entries for a list of document IDs.

        Non-blocking per document: logs a warning and continues on failure.

        Args:
            doc_ids: List of MongoDB ObjectIds of DocumentRecord documents.
            user_id: The owner's user ID (Weaviate tenant).
        ...
        for doc_id in doc_ids:
            try:
                total += await self._delete_existing(
                    self._cover_letter_weaviate, "doc_id", doc_id, user_id
                )
            except Exception:
                logger.warning(
                    "delete_cover_letter_embedding_failed",
                    doc_id=doc_id,
                    user_id=user_id,
                )
        return total
