...
async def client() -> AsyncIterator[AsyncClient]:
    ...
    with (
        ...
        patch("src.api.main.init_redis", new_callable=AsyncMock),
        patch("src.api.main.connect_weaviate", new_callable=AsyncMock),
        patch("src.api.main.close_mongodb"),
        patch("src.api.main.close_redis", new_callable=AsyncMock),
        ...
        patch("src.workflows.graphs.skills_analysis.init_skills_analysis_graph"),
        patch("src.workflows.graphs.application_assembly.init_application_assembly_graph"),
        patch("src.workflows.graphs.interview_coach.init_interview_coach_graph"),
        patch("src.workflows.graphs.negotiation.init_negotiation_graph"),
        patch("src.workflows.checkpointer.get_checkpointer"),
        patch("src.db.seeds.seed_market_configs", new_callable=AsyncMock),
    ):
        application = create_app()
        transport = ASGITransport(app=application)
        # Expose the application on the client so route tests can set
        # dependency_overrides on the correct instance (not the module-level app
        # singleton, which is a separate object created at import time).
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            ac.app = application  # type: ignore[attr-defined]
            yield ac
