...
class TestCostTrackingCallbackLedger:
    """Test that CostTrackingCallback updates the UsageLedger."""

    def _make_llm_result(self, llm_output: dict) -> LLMResult:
        return LLMResult(generations=[[]], llm_output=llm_output)

    async def test_calls_increment_usage_on_llm_end(self):
        """on_llm_end calls UsageLedgerRepository.increment_usage."""
        ...
        with (
            ...
            patch(
                "src.repositories.usage_ledger_repository.UsageLedgerRepository",
                return_value=mock_repo,
            ),
            patch("src.llm.cost_tracker.get_settings") as mock_settings,
        ):
            mock_instance = MagicMock()
            mock_instance.insert = AsyncMock()
            mock_usage_cls.return_value = mock_instance
            mock_settings.return_value.tier = MagicMock()

            await callback.on_llm_end(response)

            mock_repo.increment_usage.assert_awaited_once()
            call_kwargs = mock_repo.increment_usage.call_args[1]
            assert call_kwargs["user_id"] == "user123"
            assert call_kwargs["tokens"] == 150
            assert call_kwargs["feature"] == "cover_letter"
            assert call_kwargs["model_name"] == "claude-sonnet-4-5-20250929"
            assert call_kwargs["tier"] == UsageTier.PRO

    async def test_ledger_failure_does_not_raise(self):
        """UsageLedger write failure is logged but does not propagate."""
        callback = CostTrackingCallback(
            user_id="user123",
            agent="cover_letter_writer",
            task_type="cover_letter",
        ...
        with (
            ...
            patch("src.llm.cost_tracker.get_settings"),
        ):
            mock_instance = MagicMock()
            mock_instance.insert = AsyncMock()
            mock_usage_cls.return_value = mock_instance
    ...
    async def test_callback_accepts_tier_parameter(self):
        """CostTrackingCallback accepts optional tier parameter."""
        from src.db.documents.usage_ledger import UsageTier
        ...
        callback = CostTrackingCallback(
            ...
            agent="test",
            task_type="test",
            tier=UsageTier.ADMIN,
        )
        assert callback.tier == UsageTier.ADMIN

    async def test_callback_defaults_to_free_tier(self):
        """CostTrackingCallback defaults tier to FREE."""
        from src.db.documents.usage_ledger import UsageTier
        ...
        assert callback.tier == UsageTier.FREE
