...
# ---------------------------------------------------------------------------
# Cascade delete helpers (Story 2.2)
# ---------------------------------------------------------------------------


class TestDeleteJobEmbeddings:
    """Tests for IndexingService.delete_job_embeddings() — non-blocking Weaviate cleanup."""

    async def test_deletes_existing_job_embeddings(self, service, mock_repos):
        """Happy path: delegates to _delete_existing on job_embedding_weaviate."""
        repo = mock_repos["job_embedding_weaviate"]
        repo.search_by_property = AsyncMock(
            return_value=[{"uuid": "u1"}, {"uuid": "u2"}]
        )
        repo.delete_object = AsyncMock()

        count = await service.delete_job_embeddings(FAKE_JOB_ID, FAKE_USER_ID)

        assert count == 2
        repo.search_by_property.assert_called_once_with(
            property_name="job_id",
            property_value=FAKE_JOB_ID,
            tenant=FAKE_USER_ID,
        )
        assert repo.delete_object.call_count == 2

    async def test_returns_zero_on_exception(self, service, mock_repos):
        """Non-blocking: returns 0 and logs warning when Weaviate fails."""
        repo = mock_repos["job_embedding_weaviate"]
        repo.search_by_property = AsyncMock(side_effect=RuntimeError("connection refused"))

        count = await service.delete_job_embeddings(FAKE_JOB_ID, FAKE_USER_ID)

        assert count == 0

    async def test_returns_zero_when_no_embeddings_exist(self, service, mock_repos):
        """Returns 0 when no objects match the job_id filter."""
        repo = mock_repos["job_embedding_weaviate"]
        repo.search_by_property = AsyncMock(return_value=[])
...
class TestDeleteCoverLetterEmbeddingsForDocs:
    """Tests for IndexingService.delete_cover_letter_embeddings_for_docs()."""

    async def test_deletes_embeddings_for_each_doc(self, service, mock_repos):
        """Iterates doc_ids and deletes embeddings for each."""
        repo = mock_repos["cover_letter_weaviate"]
        repo.search_by_property = AsyncMock(
            side_effect=[
                [{"uuid": "u1"}],
                [{"uuid": "u2"}, {"uuid": "u3"}],
        ...
        count = await service.delete_cover_letter_embeddings_for_docs(
            ["doc-a", "doc-b"], FAKE_USER_ID
        )

        assert count == 3
        assert repo.search_by_property.call_count == 2
        assert repo.delete_object.call_count == 3

    async def test_continues_on_per_doc_failure(self, service, mock_repos):
        """Non-blocking per doc: logs warning and continues to next doc."""
        repo = mock_repos["cover_letter_weaviate"]
        repo.search_by_property = AsyncMock(
            side_effect=[
                RuntimeError("timeout"),          # doc-a fails
                [{"uuid": "u1"}],                 # doc-b succeeds
            ]
        )
        repo.delete_object = AsyncMock()
        ...
        # Only doc-b's embedding counted
        assert count == 1
        assert repo.delete_object.call_count == 1

    async def test_returns_zero_for_empty_doc_list(self, service):
        """Returns 0 when no doc_ids are provided."""
        count = await service.delete_cover_letter_embeddings_for_docs(
            [], FAKE_USER_ID
        )
    ...
    async def test_returns_zero_when_all_fail(self, service, mock_repos):
        """Returns 0 when every doc deletion fails."""
        repo = mock_repos["cover_letter_weaviate"]
        repo.search_by_property = AsyncMock(
